...
/* FUNCTION 29.09.2025
function updateMapFilters(selectedType, selectedFondo = null, selectedDate = null) {
    if (!map.getLayer('unclustered-point') || !map.getLayer('clusters') || !map.getLayer('cluster-count')) {
        return;
    }

    // First, filter features by date
    const filteredFeatures = filterFeaturesByDate(geojson.features, selectedDate);


    // Build the combined filter for unclustered points
    const filters = [];

    // Type filter (affects icon)
    if (selectedType) {
        filters.push(['==', ['get', 'type'], selectedType]);
    } else {
        filters.push(['!', ['has', 'point_count']]);
    }

    // Fondo filter
    if (selectedFondo) {
        filters.push(['==', ['get', 'fondo'], selectedFondo]);
    }

    if (selectedDate) {
        filters.push(['==', ['get', 'date'], selectedDate]);
    }

    const combinedFilter = filters.length > 0 ? ['all', ...filters] : ['!', ['has', 'point_count']];
    map.setFilter('unclustered-point', combinedFilter);

    // Cluster filters remain the same
    const clusterFilter = selectedType
        ? ['>', ['get', `${selectedType}_count`], 0]
        : ['has', 'point_count'];
    map.setFilter('clusters', clusterFilter);
    ...
    // Update icon-image only based on selectedType
    const iconImageExpr = selectedType
        ? `${selectedType}-icon`
...
function createTypeFilterUI(types, layer_id) {
    ...
    const createIcon = (type, label) => {
        ...
        if (type === 'all') {
            ...
            icon.innerHTML = `<img src="img/icons/${type}-icon.svg" alt="${label}" style="width: 24px;">`;
...
async function searchSimilarObjects(imageId) {
    ...
    try {
        ...
        console.error(error);
        console.log("Error directly from searchSimilarObjects()")
        ...
        .setHTML(`
            <div class="popup-content">
                ...
                <div class="popup-title">
                    <span>${title}</span>
                    <img src="img/arrow.svg" class="popup-arrow" alt="toggle arrow" />
                </div>
...
function displayResults(results) {
        ...
        <div class="featured-info">
            ...
            <table class="sidebar-info">
                <tbody>
                    ...
                    <tr><td>Collocazione:</td><td>${firstOrigin[0] || first.place || '-'}</td></tr>
                </tbody>
            </table>
        </div>
    `;
    ...
    document.querySelectorAll('#similar-items .similar-item').forEach(el => {
        el.addEventListener('click', () => {
            ...
            displayResults(id);
            const matchedFeature = geojsonData.features.find(f => String(f.properties.id) === String(id));
            if (matchedFeature) {
...
map.on('load', async () => {
    ...
    map.loadImage('img/icons-png/default-icon.png', (error, image) => {
    ...
    const response = await fetch('redis_export.geojson')//'objects_and_coordinates_18_09_2025.geojson')
